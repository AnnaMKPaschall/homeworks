...
class Widget extends React.Component {
  ...
  fetchJobListings(city) {
    let proxyUrl = 'https://cors-anywhere.herokuapp.com'; // first, forward the request to a proxy server that allows cross-origin-requests
    let jobsUrl = `https://jobs.github.com/positions.json?location=${city}&markdown=true`; //

    $.ajax({
      crossDomain: true,
      dataType: 'json', // set dataType to json since the jobs.github endpoint is json 
      url: proxyUrl + '/' + jobsUrl,
      type: "GET",
      success: function (resp) {
        this.props.store.dispatch(this.selectLocation(city, resp))
      }.bind(this)
